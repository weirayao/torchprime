...
E.g. gs://foo/bar')
def use(cluster: str, project: str, zone: str, num_slices: int, tpu_type: str,
        artifact_dir: str):
  """
  ...
  assert artifact_dir.startswith(
      "gs://"), f"{artifact_dir} must be in a GCS bucket (start with gs://)"
...
def run(args):
  ...
  ensure_command("xpk")
  xpk_command = [
      "xpk", "workload", "create", "--cluster", config.cluster,
      "--docker-image", "gcr.io/tpu-pytorch/llama3:latest", "--workload",
      f"{os.environ['USER']}-xpk-{config.tpu_type}-{config.num_slices}-{datetime_str}",
      "--tpu-type", config.tpu_type, "--num-slices",
      str(config.num_slices), "--zone", config.zone, "--project",
      config.project, "--enable-debug-logs"
  ] + env_forwarding + artifact_arg + ["--command", ' '.join(command)]
...
def ensure_command(name: str):
  ...
  try:
    ...
    raise RuntimeError(
        f"Command `{name}` not found. Make sure it is installed.") from err
