...
class TransAct(nn.Module):
  ...
  def forward(
    ...
    short_time_window_idx_trn = (request_time - action_time_seq) < rand_time_window_ms
    # use all the actions during inference
    short_time_window_idx_eval = (request_time - action_time_seq) < 0

    # adjust the mask accordingly
    if self.training:
      key_padding_mask = self._adjust_mask(key_padding_mask, short_time_window_idx_trn)
    else:
      key_padding_mask = self._adjust_mask(key_padding_mask, short_time_window_idx_eval)
