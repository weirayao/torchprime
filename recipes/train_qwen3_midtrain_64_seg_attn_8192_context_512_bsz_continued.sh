...
python torchprime/torch_xla_models/train.py \
    run_name=qwen3_midtrain_tpu_64_context_8192_full_attn_small_batch_lr_2e-4_continued \
    training_mode=pretrain \
    reshape_context=false \
    seg_attn=false \
    data=mid_train_v2_webdataset \
    model=flex-qwen-1b \
    ...
    max_steps=30000 \
    checkpoint_load_dir=gs://sfr-text-diffusion-model-research/checkpoints/midtrain_allv2/ \
    checkpoint_load_step=29500 \
    resume_from_checkpoint=false \
    checkpoint_save_dir=gs://sfr-text-diffusion-model-research/checkpoints/qwen3_midtrain_tpu_64_context_8192_full_attn_small_batch_lr_2e-4_continued \
    save_steps=1500 \
    logging_steps=1 \
    ici_mesh.fsdp=16 \
    ici_mesh.tensor=2 \
